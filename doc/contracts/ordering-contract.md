...
The customer page is `/customer/sessions/{id}`. In development, this feature uses the Vite `/api/v1` proxy to reach the local backend without depending on the shared CORS setup. Production uses `VITE_API_BASE_URL`. QR token validation, customer access control, staff authorization for menu CRUD, fulfillment status updates, billing, and real Session lookup belong to the respective owners at integration. These endpoints must be secured before deployment. API/JSON changes require ศรัณย์'s review, shared entity changes require ปวริศช์'s review, and migrations require เมธัส's review.
