...
The customer page is `/customer/sessions/{id}` and calls the shared Axios client through `VITE_API_BASE_URL`. QR token validation, customer access control, fulfillment status updates, billing, and real Session lookup belong to the respective owners at integration. Menu mutations are fail-closed by default through `MenuAdminAccessProvider`; only the `local` and `test` profiles use a permissive fixture. Authentication must provide the deployed implementation that verifies an authorized staff role. API/JSON changes require ศรัณย์'s review, shared entity changes require ปวริศช์'s review, and migrations require เมธัส's review.
