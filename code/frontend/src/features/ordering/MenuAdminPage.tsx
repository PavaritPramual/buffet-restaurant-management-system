...
const emptyItem = { categoryId: 0, name: '', description: '', available: true, packageIds: '1', imageUrl: '' }
...
export default function MenuAdminPage() {
  ...
  function submitItem(event: FormEvent) {
    ...
    const input: MenuItemInput = { categoryId: itemDraft.categoryId, name: itemDraft.name.trim(), description: itemDraft.description.trim() || null, available: itemDraft.available, packageIds: [...new Set(raw.map(Number))], imageUrl: itemDraft.imageUrl.trim() || null }
  ...
  function editItem(item: MenuItem) { setEditingItem(item.id); setItemDraft({ categoryId: item.categoryId, name: item.name, description: item.description ?? '', available: item.available, packageIds: item.packageIds.join(','), imageUrl: item.imageUrl ?? '' }); window.scrollTo({ top: 0, behavior: 'smooth' }) }
